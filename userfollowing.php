 ...
 ?>">
    <div class="navbar-inner">
	  ...
	  <?php 
		...
		$query2335w = "SELECT id,avatar,username,name
			...
			WHERE username='$look'
			";
			$query=mysqli_query($conn, $query2335w); 
		mysqli_close($conn);
	...
	if($user_id){
		...
		$as ="SELECT id,user1_id,user2_id
                              FROM following WHERE user1_id='$id' 
                         ORDER BY id DESC LIMIT 22,44
...
";
			...
			if($user_id){
				if($user_id!=$comment_user){			include 'connect.php';
			$xx = "SELECT id
								   FROM following 
			...
			if(mysqli_num_rows($query3)>=1){
				...
				if($user_id!=$comment_user){
					...
					if(mysqli_num_rows($query2)>=1){
						echo "<span><a href='unfollow.php?userid=$comment_user&username=$usernameR&ref=4&name=$look' class='button button-fill button-raised' style='background-color:teal'>Following</a></br></span>";
					}
					else{
						echo "<span><a href='follow.php?userid=$comment_user&username=$usernameR&ref=4&name=$look' class='button button-fill button-raised' style='background-color:teal'>Follow</a></br></span>";
					}
				}
			}
			echo"</div>
